                ...
                <div class="card border-0 mb-1">
                    <div class="card-body d-flex align-items-center flex-column flex-md-row">
                        ...
                        <div class="ms-md-4 m-0 mt-4 mt-md-0 text-md-start text-center w-100">
                            <h6 class="mb-3 fw-bold">{{ $services->product_name }} <span
                                    class="text-muted small fw-light d-block">Reference {{ $services->id }}</span>
                            </h6>

                            <div class="pe-xl-5 pe-md-4 ps-md-0 px-3 mb-2 d-inline-flex d-md-none">
                                <a href="{{ route('services.edit', ['service' => $services->id]) }}"
                                    class="btn btn-warning">Edit</a>
                                <form action="{{ route('services.destroy', ['service' => $services->id]) }}" method="POST"
                                    class="d-inline">
                                    @csrf
                                    @method('DELETE')
