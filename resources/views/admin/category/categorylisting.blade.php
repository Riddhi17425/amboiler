                ...
                <div class="card border-0 mb-1">
                    <div class="card-body d-flex align-items-center flex-column flex-md-row">
                        <div class="ms-md-4 m-0 mt-4 mt-md-0 text-md-start text-center w-100">
                            <h6 class="mb-3 fw-bold">{{ $category->name }}
                                <span class="text-muted small fw-light d-block">Reference {{ $category->id }}</span>
                            </h6>
                            
                            @if ($category->services->count() > 0)
                                <div class="mb-2">
                                    <label class="fw-bold small text-muted">Services</label>
                                    <ul class="list-group list-group-flush">
                                        @foreach ($category->services as $service)
                                            <li class="list-group-item px-0">{{ $service->main_title }}</li>
                                        @endforeach
                                    </ul>
                                </div>
                            @else
                                <div class="mb-2">
                                    <span class="text-muted small">No services available for this category.</span>
                                </div>
                            @endif
