...
$(document).ready(function () {
    ...
    // ✅ Hover on slide - update image + title + desc only. Click navigates normally via the <a>.
    $(document).on("mouseenter", ".service_slide_new", function () {
        updateActiveSlide($(this));
    });

    // âœ… Update when slider auto-changes
    $(".service_new_lt_tabs").on(
        "afterChange",
        function (event, slick, currentSlide) {
            const $current = $(slick.$slides[currentSlide]).find(
                ".service_slide_new"
            );
